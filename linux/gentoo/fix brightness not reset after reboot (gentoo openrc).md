...
- make a script to run at start inside in `/etc/local.d` with .start eg:`brightness.start`
example script:
```
#!/bin/sh
...
```
- make a script to run before poweroff in `/etc/local.d` with .stop eg:`brightness.stop`
example script:
```
#!/bin/sh
backlight_sys_dir="/sys/class/backlight/amdgpu_bl0" # Replace with your backlight device
